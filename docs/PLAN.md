...
- **⚠️ UI/视觉策略 — 2026-07-27 用户决议 override**：原"UI 布局非 1:1 / 绝不视觉对齐"条款**作废**。用户决定**本地构建贴近 Minecraft 1.0 的 UI 布局与中文方块命名**（底部居中 9 槽 hotbar + 上方心/饥饿排 + 中心十字；创造=全方块调色板 vs 生存=3×9 主栏+合成+护甲；中文方块名）。**硬底线保留**（arch 测试据此判定，违反=FAIL）：
  - **(a) 全部美术素材原创**：所有 GUI 构件（槽框/选框/准星/心/饥饿/面板/护甲槽/合成箭头）与方块贴图**由本项目自绘或程序生成**——**禁止拷贝 Mojang/Microsoft 的实际素材文件**（PNG/字体/贴图）。可参考 1.0 风格自绘原创像素图（QML Canvas/Rectangle 或程序生成 PNG 均可）。
  - **(b) 中文方块名用通用描述词**（草方块/泥土/石头/圆石/橡木原木/橡木木板/橡树树叶/沙子）——属描述性通用词、非商标；生物/群系/维度等**专有**名词仍按下条"概念改名表"换原创名（不逐字照搬 MC 本地化专有名）。
  - **(c) 项目名仍不含 `mine`/`craft` 语素**（不变）。
  - **(d) 公开发布前复核**：当前为本地个人项目；若将来开源/分发，发布前须重新评估 trade dress 风险，考虑切回干净呈现层。
- **"逼近 1.0"** = 机制对齐（不变）+ 本地 UI/命名贴近 1.0（本 override 范围内）。
