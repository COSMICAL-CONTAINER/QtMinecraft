...
| 段 | 范围 | 说明 |
|---|---|---|
| 方块段 | `0 .. Count-1`（`0 .. 30`） | 可放置的体素方块（含 air=0）。空气恒不可拾取 / 不可放置。 |
...
> 注：方块段 `0x1F (31) = Count` 是哨兵上界，不是合法 id。
...
## 3. 材料 / 物品段（id `0x200 .. 0x21F`，非 mob 蛋 / 非 mob 掉落部分）
...
- **方块 id 范围**：`0 .. 30`（`Count=31`，哨兵上界不可用）。
