...
**t576** 铁砧布局微调：两输入槽中间加「+」、删「放入物品与材料」提示文字
- 查：`AnvilUI.qml` A/B 槽间加号 Image/Text；删顶部提示文字。

**t577** 重命名框移到产物上方 + 放入物品自动显名 + 去掉「重命名」按钮标签
...
**t578** 铁砧放入规则：同物+对应修复材料才双格生效，不匹配出空产物
...
**t590** 附魔系统完善（附魔台选档真随机 + 装备附魔显示 + 等级/青金石消耗显示）
...
**t594** 蹒跚者改名「僵尸」问题 → 保持现名（PLAN §9 红线）；骷髅改名「骷髅弓箭手」+ 右臂贴图 + 脊柱黑色修复
...
**t595** 潜行者（苦力怕）模型比例：腿太长缩小 + 头加大 + 身体加大
