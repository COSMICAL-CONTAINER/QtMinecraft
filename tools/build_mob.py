...
def make_endereye():
    ...
    自绘非照搬 MC 资产）。透明四角 → 圆润珠感而非方形贴片。走全脸 UV（每面铺同图）；无随机源、确定性散布
    （同其它 make_* 风格）。pack 命中 ender_eye.png 时由资源包装实体贴图替身（QML 优先 pack itemIcon，回退本图）。"""
    img = Image.new("RGBA", (TS, TS), (0, 0, 0, 0))  # 透明底（圆润珠，四角透空）
