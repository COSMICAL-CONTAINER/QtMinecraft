#!/usr/bin/env python3
"""生成蘑菇（Mushroom）cross 贴图（16×16 像素，原创自绘，§9 override (a)）。

t396 沼泽群系内容：蘑菇是沼泽草地的小蘑菇装饰性 cross 广告牌方块（与草丛 / 树苗 / 枯灌木同走
cross 几何段），机制等价 MC 1.0 蘑菇（沼泽 / 阴暗草地小蘑菇）。名称 / 贴图纯原创自绘
（§9 区隔，零 MC 资产 / 专名）。

视觉意图：读作「一棵小蘑菇」—— 透明底（alpha=0）+ 一根米色菌柄 + 顶部红色菌盖（带白色斑点，
拟经典红伞白点蘑菇）。mesher 把本瓦片贴到 cross 的两片对角双面 quad；chunk 地形材质 alphaCutoff:0.5
...
输出（覆盖写入 textures/）：
  default_mushroom.png   （tile 62，蘑菇 cross 贴图）
...
def main():
    ...
    # 配色：菌柄米白（亮 / 暗）、菌盖红（亮 / 暗）、白斑（菌盖点缀）。
    stem_light = (0xe8, 0xdd, 0xc0, 255)  # 米白亮面
    stem_dark = (0xb8, 0xad, 0x90, 255)   # 米白暗面
    cap_light = (0xc4, 0x2b, 0x1c, 255)   # 砖红亮面
    cap_dark = (0x8a, 0x1c, 0x10, 255)    # 暗红暗面 / 菌盖底缘
    spot = (0xf2, 0xea, 0xd8, 255)        # 白斑（奶白）

    # 菌柄：从画布底（y=15）向上到 y=7（占下半部分），列 x=6/7（中央偏左两列，圆柱明暗）。
    for y in range(7, 16):
        put(6, y, stem_dark)
        put(7, y, stem_light)
    # 柄底略宽（蘑菇柄基部膨大）。
    ...
    # 菌盖：伞状半球，覆盖柄顶 y=3..7，水平跨度 x=3..10（宽于柄）。亮 / 暗双色给圆顶立体感。
    #   盖顶（y=3..4）窄、盖缘（y=6..7）宽 → 伞形。盖底缘用暗红描边（菌盖下侧阴影）。
    #   坐标手调呈伞状轮廓。
    cap_rows = [
    ...
    # 白斑（菌盖经典白点）：在盖面散布几个奶白小点（避开柄列 / 盖缘暗带）。
    spots = [(5, 4), (8, 4), (4, 5), (9, 5), (7, 6), (5, 6), (8, 6)]
    for (x, y) in spots:
        put(x, y, spot)

    # alpha bleed：把菌柄 / 菌盖颜色渗进透明底（alpha=0 不变），消除 cutout 黑边（同 build_sapling.py）。
    img = bleed_alpha(img)

    out = os.path.join(SRC, "default_mushroom.png")
    img.save(out)
