...
机制等价 MC 1.0 月相（8 相一周期：满→亏凸→下弦→残月→新月→蛾眉→上弦→盈凸，MC wiki 序），但贴图为本
...
相位光照模型（平面 terminator，画在正方形内）：把月视作正方形面片，光源在面内绕轴转
  ...
  i=0 α=0    : 满月（全亮）
  i=1 α=π/4  : 亏凸月（大部亮，暗缘在右下）
  i=2 α=π/2  : 下弦月（左半亮）
  i=3 α=3π/4 : 残月（左下窄亮）
  i=4 α=π    : 新月（全暗）
  i=5 α=5π/4 : 蛾眉月（右上窄亮）
  i=6 α=3π/2 : 上弦月（右半亮）
  i=7 α=7π/4 : 盈凸月（大部亮，暗缘在左上）

t612 相位序标签勘误：旧注释把 i=1..3 标成盈凸/上弦/蛾眉（按盈半球序命名），与生成贴图实测相反
  （i=1 暗部质心在右下、亮大部在左 = 亏凸；i=2 暗部右半 = 亮左半 = 下弦；i=3 仅左下窄亮 = 残月）。
  相位**环**自始正确（i 递增 = 亮面单调收缩 0满→4新、5..7 再涨回满，8 天一轮回），错的只是天文学名。
  现按 MC wiki 序更正命名（满→亏凸→下弦→残月→新月→蛾眉→上弦→盈凸）；贴图无需重生成。
