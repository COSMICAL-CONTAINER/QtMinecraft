...
QString Hotbar::iconSourceForBlock(int blockId) const
{
    ...
    // t456 pack item 图标覆盖：pack 启用且该方块在「方块→pack item/前贴图」映射内（现含 16 色床 {32..39,78..85}→
    //   bed.png + 木梯 Ladder=62→ladder.png；LapisOre=93 / CraftingTable=9 / Furnace=10 刻意不在映射，与其它矿石 / 方块
    //   一致走 3D 立方体图标 —— t493 撤销 LapisOre，t518 三轮再撤 {9,10} 用户第三次改口要 3D）、包内有 PNG 时，返 pack
    //   的 file:// URL（2D 物品图标改用 pack item 贴图，机制等价 MC item icon）；pack 关 / 无映射 / 包内缺 → 落下方
    //   程序生成 icon_<block>.png（含工作台 / 熔炉的 3D 立方体图标）。仅 2D 物品图标路径
