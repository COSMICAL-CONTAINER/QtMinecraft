...
// 工具图标（t33 / t233）：自绘原创像素工具（§9 override (a)，Canvas；非 MC 资产 PNG）。
//
// 双形状（t233 锄加入后按 toolType 分支）：
...
// 镐 / 锄共用同一 tier→色映射（材质按 tier 一致，形状按 toolType 区分）。
//
...
Item {
    ...
    property int toolType: 1 // 1=镐 Pickaxe（默认）/ 2=锄 Hoe（0 / 越界 → 兜底镐形）
    ...
    Canvas {
        ...
        onPaint: {
            ...
            // tier → 头部材质颜色（把手恒木色：通用工具柄；镐 / 锄同 tier 同色）。
            ...
            // —— 把手（镐 / 锄共用）：从左下 (4,20) 到右上 (14,10) 的对角木柄 ——
            const handleSegs = [
            ...
            // —— 头部：按 toolType 分支（镐 = 横梁 + 两端下勾；锄 = 宽扁向前伸的横刃）——
            if (root.toolType === 2) {
